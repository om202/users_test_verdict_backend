...
@app.route('/top_five_users')
def top_five_users():
    user_type = 'expert'

    #get expert list
    expert_list =  json.loads(dumps(db_users.find({"role":"expert"}, {"email":1})))
    
    labelled = {}
    for e in expert_list:
        user = e['email']
        labelled_by_user = len(json.loads(dumps(db_via.find({"Expert": user}))))
        labelled[user] = labelled_by_user
    
    #sort top five
    sorted_labelled = sorted(labelled.items(), key=lambda kv:kv[1], reverse=True)
    
    sorted_vals = []
    for s in sorted_labelled:
        sorted_vals.append(s[1])
    
    top_five = sorted_vals[:5]

    # masked users
    masked_users = {}
    j = 0
    for i in top_five:
        masked_users['user'+str(j)] = i
        j = j+1
...
def expert_feed_progress_bar():
    ...
    if expert_for == 0:
        ...
        done = json.loads(dumps(db_cca.find({"Expert": f"{expert}"})))
        done_count = len(done);
        return {'total': count, 'done': done_count}
    if expert_for == 1:
        done = json.loads(dumps(db_via.find({"Expert": f"{expert}"})))
        done_count = len(done);
